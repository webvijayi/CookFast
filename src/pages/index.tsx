...
// --- SVG Icons ---
const GitHubIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="inline-block ml-1" viewBox="0 0 16 16">
...
export default function CookFastHome() {
  ...
  const handleValidateKey = async () => {
    ...
    try {
       ...
       if (response.ok && data.valid) { setKeyValidationStatus('valid'); } else { setKeyValidationStatus('invalid'); setKeyValidationError(data.error || "Validation failed."); }
    } catch (err) {
        setKeyValidationStatus('invalid'); setKeyValidationError(err instanceof Error ? err.message : "Network/server error during validation.");
